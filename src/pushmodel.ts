...
export function mount(server: Server, path: string, model: any, acceptOrigins: Array<string>) {
	...
	wss.on("connection", function(ws: WebSocket) {
		...
		async function onReceive(text: string) {
...
class RequestHandler {
	...
	async handle(text: string) {
		try {
			...
			for (const request of requests) await this.handleRequest(request);
		}
		catch (err) {
			console.error(err);
	...
	async handleRequest(request: RpcRequest) {
		...
		try {
			const result: any = await func.apply(this.model, request.params||[]);
			if (result instanceof ErrorResponse) this.sendError(request.id, result.code, result.message, result.data);
